...
// Fungsi Komponen Task Card (Di sini, Completed tasks diberi style yang sedikit berbeda)
const TaskCard: React.FC<{ task: Task, isDarkMode: boolean }> = ({ task, isDarkMode }) => {
    ...
    // Style khusus untuk Completed Task: sedikit pudar (opacity 70%) dan garis miring
    const isCompleted = task.status === 'Completed';
    
    const cardBaseClasses = isDarkMode 
        ? `bg-gray-800 border-gray-700 ${isCompleted ? 'opacity-70' : 'hover:bg-gray-700/80'}` 
        : `bg-white border-gray-200 ${isCompleted ? 'opacity-70' : 'hover:shadow-md hover:border-blue-300'}`;
    ...
    return (
        <div className={`p-4 border rounded-xl transition-all duration-300 ${cardBaseClasses} flex justify-between items-start gap-4`}>
            <div className="flex items-start flex-grow min-w-0">
                {/* Ikon centang berwarna biru jika Completed */}
                <button className={`mt-1 p-0.5 rounded-full transition-colors ${isCompleted ? 'text-blue-500' : isDarkMode ? 'text-gray-500 hover:text-blue-400' : 'text-gray-400 hover:text-blue-600'}`}>
                    <CheckCircleIcon className="w-6 h-6" fill={isCompleted ? 'currentColor' : 'none'} />
                </button>
                <div className="ml-3 min-w-0">
                    <h3 className={`text-base font-semibold truncate ${isCompleted ? 'line-through' : ''} ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{task.title}</h3>
                    <p className={`text-xs ${textMutedClasses} mt-1`}>Completed on: {task.dueDate}</p>
            ...
            <button className={`p-2 rounded-full transition-colors ${isDarkMode ? 'text-gray-500 hover:text-white' : 'text-gray-500 hover:bg-gray-100'}`} disabled={isCompleted}>
...
const Completed: React.FC = () => {
    ...
    // Filter tasks hanya yang berstatus 'Completed'
    ...
    const tabClasses = (tab: string) => 
        `px-4 py-2 text-sm font-semibold rounded-lg transition-colors duration-300 ${isDarkMode 
            ? (tab === 'Completed' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/40' : 'text-gray-300 hover:bg-gray-700') 
            : (tab === 'Completed' ? 'bg-blue-600 text-white shadow-lg shadow-blue-300/60' : 'text-gray-700 hover:bg-gray-200')
        }`;
        ...
        <div className={`min-h-screen transition-colors duration-500 ${containerClasses} p-4 sm:p-8 flex justify-center`}>
            ...
            <div className={`w-full max-w-lg ${cardBgClasses} rounded-2xl p-6 md:p-8 transition-colors duration-500`}>
                ...
                <div className="flex space-x-2 overflow-x-auto pb-4 mb-6">
                    ...
                    <button className={tabClasses('Completed')} onClick={() => {}}>Completed</button>
                </div>
                ...
                <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-2">
                    {completedTasks.map(task => (
                        // Completed tasks menggunakan TaskCard yang sama, yang memiliki logika visual untuk Completed
                        <TaskCard key={task.id} task={{...task, status: 'Completed'}} isDarkMode={isDarkMode} /> 
                    ))}
                    
                    {completedTasks.length === 0 && (
                        <p className={`text-center py-10 ${subHeaderTextClasses}`}>No tasks have been completed yet.</p>
                    )}
                </div>

                {/* Tombol Tambah Tugas (disembunyikan untuk Completed sesuai desain, tapi saya biarkan di sini sebagai placeholder) */}
                {/* <div className="mt-8">
